...
The format follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and `fallow`
...
- `fallow scan` against a GCP project, folder, or organization, scoped with
  `--gcp-project`, `--gcp-folder`, or `--gcp-organization` (or `FALLOW_GCP_PROJECT`,
  `FALLOW_GCP_FOLDER`, `FALLOW_GCP_ORGANIZATION`). Flags take precedence.
- Authentication via Application Default Credentials only. `fallow` reads no key files and
...
- `fallow-exempt=true` as a resource label to exclude a resource from every rule.
...
- `fallow graph export` to dump a graph snapshot as JSON.
...
- `fallow rules list` and `fallow rules explain <id>` for the rule catalogue.
- `fallow version`, reporting the stamped version plus the commit and build time from the
...
[Unreleased]: https://github.com/TypeOneLabs/fallow/compare/v0.1.0...HEAD
[0.1.0]: https://github.com/TypeOneLabs/fallow/releases/tag/v0.1.0
