...
// graph as a JSON snapshot that `fallow scan --cache-file` replays verbatim.
//
...
func newGraphExportCmd(g *globalFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Ingest the scope's asset inventory, enrich it with Cloud Monitoring " +
			"series (exactly as `fallow scan` does), and write the result as a JSON " +
			"graph snapshot. `fallow scan --cache-file THIS_FILE` then replays it " +
			...
			"to evaluate.",
		Example: "  fallow graph export --gcp-project p --out graph.json\n" +
			"  fallow scan --cache-file graph.json   # full-fidelity replay",
		Args: cobra.NoArgs,
