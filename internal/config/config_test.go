...
import (
	...
	_ "github.com/TypeOneLabs/fallow/pkg/cloud/gcp" // registers FALLOW_GCP_* via init()
)
...
func TestResolveScope_OtherProviderScopeNotPickedUp(t *testing.T) {
	t.Setenv("FALLOW_GCP_PROJECT", "gcp-value")
	t.Setenv("FALLOW_OTHER_PROJECT", "other-value")

	if got := resolveScope("gcp", scopeProject, ""); got != "gcp-value" {
		t.Fatalf("resolveScope(gcp) = %q, want %q — must not read FALLOW_OTHER_PROJECT", got, "gcp-value")
	}
	if got := resolveScope(otherProviderName, scopeProject, ""); got != "other-value" {
		t.Fatalf("resolveScope(other) = %q, want %q — must not read FALLOW_GCP_PROJECT", got, "other-value")
	}
}
...
func TestResolveScope_ScopeNotAcceptedByProviderYieldsEmpty(t *testing.T) {
	t.Setenv("FALLOW_GCP_FOLDER", "gcp-folder")
