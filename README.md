...
```bash
git clone https://github.com/TypeOneLabs/fallow.git && cd fallow
go build -o fallow ./cmd/fallow

gcloud auth application-default login
./fallow scan --gcp-project my-project
```
...
No credentials at hand? `./fallow scan --gcp-project demo --fixture assets.json` runs the
...
everything into their warehouse, and you rent a dashboard. `fallow` is the opposite bet —
...
- **Live GCP ingestion** — Cloud Asset Inventory via `cloud.google.com/go/asset`, scoped to
  ...
  fallow prices — a waste figure is only honest if it reflects what the scope looks like
...
- **Scan artifacts** — every scan leaves a directory of artifacts behind in `--out-dir`
  (default `fallow-out/`): a replayable graph snapshot, the findings JSON, and a
...
metrics. `fallow rules list` shows the catalogue; `fallow rules explain <id>` prints a
...
That is a development build: `fallow version` reports `dev` for the version, and the commit
and build time come from the Go toolchain's own VCS stamps. For a release build, stamp the
version too:

```bash
go build -ldflags "-X github.com/TypeOneLabs/fallow/internal/buildinfo.Version=v0.1.0" \
  -o fallow ./cmd/fallow
```
...
```bash
fallow --help                 # top-level commands
fallow rules list             # the rule catalogue
fallow rules explain detached_disk
fallow version
fallow graph export --gcp-project my-project --out graph.json   # enrich + dump a graph snapshot
```

### Scanning a real project

Authenticate once with Application Default Credentials — `fallow` reads no key files and
...
environment variables `FALLOW_GCP_PROJECT`, `FALLOW_GCP_FOLDER`, `FALLOW_GCP_ORGANIZATION`.
...
A scan is not just a table on stdout. Every `fallow scan` (online or offline, real or
fixture) leaves a directory of artifacts behind, so a run you cannot watch still records
what it saw and found. The directory defaults to `fallow-out/` and the artifacts land in a
...
```bash
fallow scan --gcp-project my-project --out-dir ./reports/
```
...
- **`graph-<scope>.json`** — the enriched graph snapshot. It is written by the exact same
  ...
  `fallow scan --cache-file fallow-out/<scope>-<stamp>/graph-<scope>.json` reproduces the
...
different fidelity, and fallow says so explicitly at the end of every offline scan.**
...
fallow does not let this look like "no waste". At the end of a fixture run, the report
states explicitly which rules could not be evaluated for lack of metrics:

```bash
fallow scan --gcp-project my-project --fixture assets.json
```
...
fallow scan --gcp-project my-project --cache-file snap.json

# Later runs: graph replayed verbatim — zero API calls, same rules, same data.
fallow scan --cache-file snap.json
```
...
`SearchAllResources` RPC fallow's live path calls). The command below captures every asset
type the shipped rules request, with the full versioned resource payload, and writes a
plain JSON array that `fallow scan --fixture` consumes **without any hand-editing**:
...
fallow's fixture reader accepts (it folds `versionedResources` onto the `RawAsset` shape
...
raw inventory; a `fallow graph export` (below) writes an enriched graph that carries metric
...
```bash
fallow graph export --gcp-project my-project --out graph.json
fallow scan --cache-file graph.json
```
...
```bash
fallow scan --gcp-project my-project --fixture assets.json --format json
fallow scan --gcp-project my-project --fixture assets.json --format csv
```
...
| `--out-dir` | Directory to write scan artifacts into (created if absent; default `fallow-out/`) |
...
Release history is in [CHANGELOG.md](CHANGELOG.md). `fallow` follows
...
`fallow` is open source and stays that way. TypeOne Labs also builds commercial software
