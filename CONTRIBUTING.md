...
Rules are the most useful thing you can add. `fallow` ships three, and the interesting ones
...
- **Ships a fixture.** Add a Cloud Asset Inventory JSON fixture that triggers your rule, so
  `fallow scan --fixture` reproduces the finding with no cloud account. This is how reviewers
  verify your work, and it's why offline mode exists.
- **Honours `fallow-exempt=true`** on a resource, checked before anything else.
...
`fallow` is [Apache 2.0](LICENSE), and contributions are made under the same licence. That's
