...
// GKE/Cloud Run metadata server, in that order. fallow never reads a key file
...
// resource is always populated for the asset types fallow models.
//
...
func toRawSearchResult(r *assetpb.ResourceSearchResult) *RawAsset {
	...
	for _, vr := range r.GetVersionedResources() {
		...
		// version. Take the first populated one — for the types fallow models
