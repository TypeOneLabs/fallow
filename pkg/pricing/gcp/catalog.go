...
// here in UnitPrice, and stated in `fallow scan --help` (see the --price-file
...
// large (thousands of SKUs across the handful of services fallow prices) and
...
// table rather than stalling the rule. sku is fallow's internal SKU token
...
// resource name for each display name fallow needs.
...
// matchSKU derives fallow's (Kind, sku-token) pair from a Cloud Billing SKU's
...
// for every SKU fallow does not model - the vast majority of the catalogue.
...
// fallow keeps money in integer cents on the cost path, so the conversion
...
func unitPriceOf(sk *billingpb.Sku) (float64, bool) {
	...
	// fallow's cost model is linear, not tiered (matching every embedded
