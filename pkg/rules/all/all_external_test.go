...
import (
	...
	_ "github.com/TypeOneLabs/fallow/pkg/rules/gcp/compute/detached_disk"
	_ "github.com/TypeOneLabs/fallow/pkg/rules/gcp/compute/underutilized_instance"
	_ "github.com/TypeOneLabs/fallow/pkg/rules/gcp/compute/unused_reserved_ip"
	_ "github.com/TypeOneLabs/fallow/pkg/rules/gcp/gcs/no_lifecycle_policy"

	_ "github.com/TypeOneLabs/fallow/pkg/rules/all" // what `all` actually ships
...
func importedRulePackages(t *testing.T) []string {
	t.Helper()
	pkg, err := build.Import("github.com/TypeOneLabs/fallow/pkg/rules/all", "", 0)
	if err != nil {
		t.Fatalf("build.Import(pkg/rules/all): %v", err)
	}

	ruleRoot := "github.com/TypeOneLabs/fallow/pkg/rules/"
	var out []string
	for _, imp := range pkg.Imports {
		if strings.HasPrefix(imp, ruleRoot) && imp != "github.com/TypeOneLabs/fallow/pkg/rules" {
			out = append(out, imp)
		}
	}
	sort.Strings(out)
