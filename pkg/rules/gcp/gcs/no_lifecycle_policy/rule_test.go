...
// TestEval_FallowExemptLabel_Skips is the P0 short-circuit: even a perfectly
// valid firing bucket carrying fallow-exempt=true must skip for the label and
// never reach any other predicate.
func TestEval_FallowExemptLabel_Skips(t *testing.T) {
	n := bucketNode(float64(100 << 30))
	n.Labels = map[string]string{"fallow-exempt": "true"}
	findings, skips := runEval(t, []*graph.Node{n}, defaultPricer())
